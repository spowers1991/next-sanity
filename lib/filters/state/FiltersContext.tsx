...
interface FiltersContextType {
  STATE_itemsToFilter: FiltersItem[];
  STATE_setItemsToFilter: React.Dispatch<React.SetStateAction<FiltersItem[]>>;
  ...
  STATE_filteredItems: FiltersItem[];
  STATE_setFilteredItems: React.Dispatch<React.SetStateAction<FiltersItem[]>>;
...
export const FiltersProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [STATE_itemsToFilter, STATE_setItemsToFilter] = useState<FiltersItem[]>([]);
  const [STATE_filtersOptions, STATE_setFiltersOptions] = useState<Record<string, string[]>>({});
  const [STATE_filteredItems, STATE_setFilteredItems] = useState<FiltersItem[]>([]);
